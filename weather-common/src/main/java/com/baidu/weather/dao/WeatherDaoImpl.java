...
public class WeatherDaoImpl implements WeatherDao {
    ...
    public void insertWeather(Map<String, Object> weather) {
        String sql;
        int id = queryIdByCitycode("weather", weather.get("citycode").toString(), weather.get("source").toString());
        if (id == -1) {
    ...
    public void insertAir(Map<String, Object> air) {
        String sql;
        int id = queryIdByCitycode("air", air.get("citycode").toString(), air.get("source").toString());
        if (id == -1) {
            SimpleJdbcInsert simpleJdbcInsert = new SimpleJdbcInsert(jdbcTemplate);
    ...
    public void insertFuture(Future future) {
        ...
        if (!future.getWindScaleDay().isEmpty()) {
            fut.put("wind_scale_day", future.getWindScaleDay());
        }
        fut.put("wind_scale_night", future.getWindScaleNight());
        ...
        String sql;
        long id = queryIdFromFutureByCitycodeAndDate(future.getCityId(), future.getDate(), future.getSource());
        if (id == -1) {
            SimpleJdbcInsert simpleJdbcInsert = new SimpleJdbcInsert(jdbcTemplate);
    ...
    private long queryIdFromFutureByCitycodeAndDate(String citycode, String date, String source) {
        String sql = "select id from future where citycode=? and date=? and source=?";
        long id = -1;
        try {
            id = jdbcTemplate.queryForObject(sql, new Object[]{citycode, date, source}, Long.class);
    ...
    public void insertSug(Map<String, Object> suggestion) {
        ...
        sug.put("type", suggestion.get("type"));
        ...
        int id = queryIdFromSug(suggestion.get("citycode").toString(), suggestion.get("type").toString(), suggestion.get("source").toString());
        String sql;
        if (id == -1) {
            SimpleJdbcInsert simpleJdbcInsert = new SimpleJdbcInsert(jdbcTemplate);
    ...
    private int queryIdFromSug(String citycode, String type, String source) {
        String sql = "select id from sug where citycode=? and type=? and source=?";
        int key = -1;
        try {
            key = jdbcTemplate.queryForObject(sql, new Object[]{citycode, type, source}, new int[]{Types.VARCHAR, Types.VARCHAR, Types.VARCHAR}, Integer.class);
        } catch (EmptyResultDataAccessException e) {
            key = -1;
        } catch (DataAccessException e) {
    ...
    private int queryIdByCitycode(String tableName, String citycode, String source) {
        String sql = "select id from " + tableName + " where citycode=? and source=?";
        int id = -1;
        try {
            id = jdbcTemplate.queryForObject(sql, new Object[]{citycode, source}, new int[]{Types.VARCHAR, Types.VARCHAR}, Integer.class);
        } catch (EmptyResultDataAccessException e) {
            id = -1;
        } catch (DataAccessException e) {
    ...
    private String processUpdateSql(Map<String, Object> map, String tableName, long id) {
    ...
    public List<City> getCities() {
        ...
        SqlRowSet set = namedParameterJdbcTemplate.queryForRowSet("select citycode,areacode,cityname,areaname,prov from city", paramMap);
        while (set.next()) {
            City city = new City(set.getString("citycode"), set.getString("areacode"), set.getString("cityname"), set.getString("areaname"), set.getString("prov"));
            result.add(city);
        }
        return result;
    }
    ...
    @Override
    public List<City> getUSAAQICities() {
        List<City> result = new ArrayList<>();
        Map<String, ?> paramMap = new HashMap<>();
        SqlRowSet set = namedParameterJdbcTemplate.queryForRowSet("select * from usa_aqi", paramMap);
        while (set.next()) {
            City city = new City(set.getString("citycode"), set.getString("areacode"), set.getString("cityname"), set.getString("areaname"), null, set.getString("citynameen"));
            result.add(city);
        }
        return result;
    }
    ...
    @Override
    public void deleteFuture(String pubdate) {
        String sql = "delete from future where pubdate<?";
        jdbcTemplate.update(sql, new Object[]{pubdate});
    }

    @Override
    public void deleteAlarm(String pubdate) {
        String sql1 = "delete from warning_raw where pubdate<?";
        String sql2 = "delete from warning_publish where pubdate<?";
        jdbcTemplate.update(sql1, new Object[]{pubdate});
        jdbcTemplate.update(sql2, new Object[]{pubdate});
    }
}
